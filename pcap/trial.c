...
static struct ether_addr spoof_mac, target_mac, hax_mac;
...
usage(void)
{
	fprintf(stderr, "Version: " VERSION "\n"
		"Usage: arpspoof [-i interface] [-s spoofmac] [-t target] host\nspoofmac is the hardware address you wish to spoof for the host. By default this is your local mac address");
	exit(1);
}
...
main(int argc, char *argv[])
{
	...
	bool set_hax_mac = false;
	
	intf = NULL;
	spoof_ip = target_ip = 0;
	
	while ((c = getopt(argc, argv, "i:t:s:h?V")) != -1) {
		switch (c) {
		case 'i':
			intf = optarg;
			break;
	...
	if(hax_mac == NULL && set_hax_mac == true)
		usage();
	...
	for (;;) {
		arp_send(llif, intf, ARPOP_REPLY, (set_hax_mac ? (u_char *)&hax_mac : NULL), spoof_ip,
			 (target_ip ? (u_char *)&target_mac : NULL),
			 target_ip);
		sleep(2);
	}
