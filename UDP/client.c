...
void * reader(void *f)
{
	while(1)
	{
		...
		int s = recvfrom(sfd,buff,50,0,(struct sockaddr*)&saddr,&d);
		if(s>0){
...
void * writer(void *f)
{
	while(1)
	{
		...
		int p=(int)ntohs(saddr.sin_port);
		printf("%d\n",p);
		int g = sendto(sfd,buffer,strlen(buffer),0,(struct sockaddr*)&saddr, sizeof(saddr));
		printf("g=%d\n",g );
		sleep(1);
	}
}
